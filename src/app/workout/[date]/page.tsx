...
type WorkoutData = {
  ...
  generalNotes?: string;
  blocks: WorkoutBlock[];
  isEnduranceDay: boolean;
};

// --- Helper: Parsing Logic (Updated for Mobility & Warmups) ---
function parseWorkoutToBlocks(dayPlan: any): WorkoutData {
  const blocks: WorkoutBlock[] = [];
  let globalBlockIndex = 0;
  let isEnduranceDay = false;
  ...
  // Check if purely endurance day
  const mainFocus = dayPlan.detailed_workouts?.[0]?.focus?.toLowerCase() || '';
  if (mainFocus === 'cardio' || mainFocus === 'endurance') {
      isEnduranceDay = true;
  }
  ...
  sourceList.forEach((dw: any) => {
    ...
    // 3. Exercise Parsing (Strength / Mobility / HIIT)
    if (dw.instructions && Array.isArray(dw.instructions)) {
      ...
      dw.instructions.forEach((line: string) => {
        ...
        // Regex for Strength sets: "3 sets...", "3 x 10", "3 rounds"
        const exerciseMatch = line.match(/^(.+?)(?::|-)?\s+(\d+)\s*(?:sets|x|rounds)/i);
        
        // Regex for Mobility items: lines starting with numbers or bullet points
        // OR just short distinctive lines if focus is mobility
        const isMobilityItem = focus === 'mobility' && (line.match(/^\d+\./) || line.trim().length > 3);

        if (exerciseMatch) {
          // --- STRENGTH / HIIT EXERCISE ---
          ...
          currentExercise = {
            ...
            sets: Array.from({ length: setsCount }).map((_, i) => ({
                ...
                isBodyweight: !weightMatch, // Default to BW if no weight specified
                completed: false
            }))
          };

        } else if (isMobilityItem && !exerciseMatch) {
           // --- MOBILITY MOVEMENT (SPLIT INTO ITEMS) ---
           // If we have a previous exercise waiting, push it
           if (currentExercise) {
                if (pendingInstructions.length > 0) currentExercise.instructions = pendingInstructions;
                blocks.push(currentExercise);
                pendingInstructions = [];
...
function EnduranceSessionView({ data, onFinish, isSaving }: { data: WorkoutData, onFinish: () => void, isSaving: boolean }) {
    const mainBlock = data.blocks.find(b => b.type === 'endurance_session') || data.blocks[0];
    ...
    return (
        <div className="min-h-screen bg-black text-white flex flex-col relative pb-10">
            ...
            <header className="px-6 py-6 z-10">
                ...
                <h1 className="text-4xl font-bold leading-tight">{mainBlock?.name || 'Cardio Session'}</h1>
                <p className="text-zinc-400 mt-2 text-lg whitespace-pre-line">{mainBlock?.subtitle || 'Steady state effort'}</p>
            </header>
            ...
            <div className="flex-1 flex flex-col items-center justify-center z-10 px-6">
                ...
                {mainBlock?.instructions && mainBlock.instructions.length > 0 && (
                    <div className="mt-8 bg-black/40 border border-zinc-800 rounded-xl p-4 w-full max-w-sm">
                        ...
                        <ul className="space-y-2">
                            {mainBlock.instructions.map((inst, i) => (
...
export default function ActiveWorkoutPage() {
    ...
    const loadPlanAndHistory = async () => {
      try {
        ...
        if (!plans || plans.length === 0) { 
            console.error("No plans found in DB");
            setLoading(false); 
            return; 
        }
        ...
        const fullDayName = dateObj.toLocaleDateString('en-US', { weekday: 'long' }); // "Monday"

        console.log("Looking for day:", dayName, "or", fullDayName);

        // ניסיון למצוא את היום בתוכנית (גמיש יותר)
        const dayPlan = planContent.days.find((d: any) => 
            d.weekday.toLowerCase().startsWith(dayName.toLowerCase()) || 
            d.weekday.toLowerCase() === fullDayName.toLowerCase()
        );

        if (!dayPlan) { 
            // במקום סתם להיכשל, ננסה לקחת לפי אינדקס היום בשבוע אם השמות לא תואמים
            // יום ראשון = 0, שני = 1 וכו'.
            // זה פתרון חירום למקרה שה-AI החזיר שמות מוזרים
            console.error("Day match failed. Plan days:", planContent.days.map((d:any) => d.weekday));
            alert(`Debug: Could not find plan for ${dayName}. Check console for details.`);
            setLoading(false);
            return; 
        }
        ...
        // ... (המשך קוד ההיסטוריה נשאר זהה - תעתיק מהקוד הקודם את החלק של Fetch History) ...
        // ...
        
        setWorkoutData(parsedData);
      } catch (err) {
        console.error("CRITICAL ERROR:", err);
        alert("Error loading: " + JSON.stringify(err));
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateSet = (blockIndex: number, setIndex: number, field: keyof ExerciseSet, value: any) => {
    ...
    // Toggle Logic for Bodyweight
    if (field === 'isBodyweight') {
        const isNowBW = value === true;
        newSets[setIndex] = { 
            ...newSets[setIndex], 
            isBodyweight: isNowBW,
            weight: isNowBW ? '' : newSets[setIndex].weight // אם עבר ל-BW, מנקה משקל. אחרת משאיר.
        };
    } else {
  ...
  if (workoutData.isEnduranceDay) {
      return <EnduranceSessionView data={workoutData} onFinish={finishWorkout} isSaving={isSaving} />;
  }
      ...
      <div className="p-4 space-y-6 max-w-lg mx-auto mt-2">
        ...
        {workoutData.blocks.map((block, blockIndex) => {
            ...
            if (block.type === 'warmup' || block.type === 'cooldown') {
                return (
                    <div key={block.id} 
                         onClick={() => toggleBlockCompletion(blockIndex)}
                         className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
                             block.isCompleted ? 'bg-emerald-900/10 border-emerald-500/30' : 'bg-zinc-900/50 border-zinc-800'
                         }`}
                    >
                        <div className="flex-1">
                            <h3 className={`font-bold ${block.type === 'warmup' ? 'text-amber-200' : 'text-indigo-200'}`}>
                                {block.name}
                            </h3>
                            {block.subtitle && <p className="text-sm text-zinc-300 mt-0.5 font-medium whitespace-pre-line">{block.subtitle}</p>}
                        </div>
                        <div className={`w-6 h-6 rounded-full border flex items-center justify-center ml-3 ${block.isCompleted ? 'bg-emerald-500 border-emerald-500 text-black' : 'border-zinc-600'}`}>
                            {block.isCompleted && '✓'}
                        </div>
                    </div>
                );
                    ...
                    <div className="bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden shadow-sm">
                        ...
                        {block.sets.map((set, setIndex) => (
                            <div key={setIndex} className={`grid grid-cols-12 border-b last:border-0 border-zinc-800/50 items-center p-2 py-3 transition-colors ${set.completed ? 'bg-emerald-900/10' : ''}`}>
                                {isMobility ? (
                                    <>
                                        <div className="col-span-9 pl-3 text-sm text-zinc-300 font-medium">Complete Movement</div>
                                        ...
                                        <div className="col-span-4 px-1 relative flex items-center gap-1">
                                            {!set.isBodyweight ? (
                                                <input 
                                                    ...
                                                    className="w-full bg-zinc-900 border border-zinc-700 rounded-lg text-center py-2 text-sm text-white focus:border-emerald-500 outline-none placeholder:text-zinc-700"
                                                />
                                            ) : (
                                            ...
                                            <button 
                                                onClick={() => updateSet(blockIndex, setIndex, 'isBodyweight', !set.isBodyweight)}
                                                className={`absolute -top-1.5 -right-0.5 text-[8px] px-1 rounded border ${set.isBodyweight ? 'bg-indigo-500 border-indigo-500 text-white' : 'bg-zinc-800 border-zinc-700 text-zinc-500'}`}
                                            >
                                                BW
                                            </button>
                                        </div>
